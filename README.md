...
1. **Fine-Tuned Specialized Transformer:** A shallow pre-trained language model (**DistilBERT**) trained locally using 5-Fold Cross-Validation and an Ensemble inference strategy.
...
- **Training Strategy:** 5-Fold Cross-Validation (Stratified by label combinations). To circumvent hardware bottlenecks and speed up local iterations, severe downsampling was applied (8,000 total rows: 3,000 toxic, 5,000 clean).
...
```text
├── my_dataset.py            # Modular PyTorch Dataset wrapper
├── bert_base.ipynb          # DistilBERT fine-tuning, 5-Fold CV, and Ensemble inference
├── local_llm.ipynb          # Local Ollama connector for Zero-Shot and Few-Shot evaluation
├── .gitignore               # Shields heavy files (.pt, .csv) and presentation artifacts
...
```bash
pip install torch transformers scikit-learn pandas tqdm requests ollama
...
3. Run `bert_base.ipynb` for DistilBERT training and ensemble inference, and `local_llm.ipynb` for LLM evaluation against `mini_test_sample.csv` to replicate the benchmark reports.
